...
const defaultApiServerUrl = "https://pulih-api.salmanabdurrahman.my.id";
...
const schemas = {
  ...
  PsychologistProfile: {
    ...
    properties: { id: uuid(examples.psychologistId), userId: uuid(examples.userId), type: { type: "string", enum: ["general", "clinical"], example: "clinical" }, consultationChannel: { type: "string", enum: ["chat", "chat_and_meet"], example: "chat_and_meet" }, fullName: { type: "string", example: "Dr. Demo" }, approvalStatus: { type: "string", enum: ["draft", "pending_review", "approved", "rejected", "suspended"], example: "approved" }, dateOfBirth: { type: "string", format: "date", example: "1990-01-01" }, address: { type: "string", example: "Jl. Demo No. 1" }, photoUrl: { type: "string", format: "uri", example: "https://example.com/photo.jpg" }, bio: nullableString("Licensed clinical psychologist."), ratingSummary: { type: "object", required: ["averageRating", "reviewCount"], properties: { averageRating: { type: "number", example: 4.8 }, reviewCount: { type: "integer", example: 12 } } }, latestReviews: { type: "array", maxItems: 5, items: ref("BookingReview") }, latestBundle: refOrNull("SessionBundle") },
  },
  PsychologistProfileWithEmail: {
    type: "object",
    required: ["id", "userId", "email", "type", "consultationChannel", "approvalStatus", "fullName", "dateOfBirth", "address", "photoUrl", "bio", "ratingSummary", "latestReviews", "latestBundle"],
    properties: { id: uuid(examples.psychologistId), userId: uuid(examples.userId), email: { type: "string", format: "email", example: "psychologist@example.com" }, type: { type: "string", enum: ["general", "clinical"], example: "clinical" }, consultationChannel: { type: "string", enum: ["chat", "chat_and_meet"], example: "chat_and_meet" }, fullName: { type: "string", example: "Dr. Demo" }, approvalStatus: { type: "string", enum: ["draft", "pending_review", "approved", "rejected", "suspended"], example: "approved" }, dateOfBirth: { type: "string", format: "date", example: "1990-01-01" }, address: { type: "string", example: "Jl. Demo No. 1" }, photoUrl: { type: "string", format: "uri", example: "https://example.com/photo.jpg" }, bio: nullableString("Licensed clinical psychologist."), ratingSummary: { type: "object", required: ["averageRating", "reviewCount"], properties: { averageRating: { type: "number", example: 4.8 }, reviewCount: { type: "integer", example: 12 } } }, latestReviews: { type: "array", maxItems: 5, items: ref("BookingReview") }, latestBundle: refOrNull("SessionBundle") },
  },
  PsychologistSession: {
    type: "object",
  ...
  CredentialReviewUrl: {
    ...
    properties: { fileId: uuid(examples.fileId), reviewUrl: { type: ["string", "null"], format: "uri", example: "https://storage.example.local/credential.pdf" }, expiresAt: nullableString("2026-07-16T10:15:00.000Z", { format: "date-time" }), message: { type: "string", example: "Signed review URL is not configured. Use storage dashboard/manual operations for private review." } },
  },
  SessionBundle: {
    type: "object",
  ...
  AvailabilityWindowResult: {
    type: "object",
    required: ["bundles", "sessions"],
    properties: { bundles: arr(ref("SessionBundle")), sessions: arr(ref("PsychologistSession")) },
  },
  PublicPsychologist: {
    type: "object",
    required: ["id", "userId", "type", "consultationChannel", "approvalStatus", "fullName", "dateOfBirth", "address", "photoUrl", "bio", "ratingSummary", "latestReviews", "latestBundle"],
  ...
  Payment: {
    ...
    properties: { id: uuid(examples.paymentId), bookingId: uuid(examples.bookingId), provider: { type: "string", enum: ["pakasir"], example: "pakasir" }, orderId: { type: "string", example: "PLH-20260201T070000-ABCDEF12" }, amount: { type: "integer", example: 150000 }, status: { type: "string", enum: ["created", "pending", "completed", "failed", "expired", "cancelled"], example: "created" }, paymentMethod: nullableString("qris"), paymentUrl: nullableString("https://app.pakasir.com/pay/pulih/150000?order_id=PLH-20260201T070000-ABCDEF12&redirect=http%3A%2F%2Flocalhost%3A3001%2Fconsultation%2Fbooking%2Fsuccess", { format: "uri" }), completedAt: nullableString(null, { format: "date-time" }), expiresAt: dateTime("2026-07-16T17:00:00.000Z"), createdAt: dateTime("2026-07-16T16:00:00.000Z"), updatedAt: dateTime("2026-07-16T16:00:00.000Z") },
  },
  PsychologistAvailabilityDate: {
    type: "object",
    required: ["date", "totalSlots", "availableSlots", "heldSlots", "bookedSlots", "completedSlots", "cancelledSlots", "expiredSlots", "rescheduledSlots", "slots"],
    properties: {
      date: date("2026-06-03"),
      totalSlots: { type: "integer", minimum: 0, example: 3 },
      availableSlots: { type: "integer", minimum: 0, example: 1 },
      heldSlots: { type: "integer", minimum: 0, example: 0 },
      bookedSlots: { type: "integer", minimum: 0, example: 2 },
      completedSlots: { type: "integer", minimum: 0, example: 0 },
      cancelledSlots: { type: "integer", minimum: 0, example: 0 },
      expiredSlots: { type: "integer", minimum: 0, example: 0 },
      rescheduledSlots: { type: "integer", minimum: 0, example: 0 },
      slots: { type: "array", items: { type: "object", required: ["id", "startsAt", "endsAt", "status", "packageName", "packageDurationMinutes", "priceAmount"], properties: { id: uuid(examples.sessionSlotId), startsAt: dateTime("2026-06-03T09:00:00.000Z"), endsAt: dateTime("2026-06-03T12:00:00.000Z"), status: { type: "string", enum: ["available", "held", "booked", "completed", "cancelled", "expired", "rescheduled"], example: "available" }, packageName: { type: "string", example: "Paket 3 Jam" }, packageDurationMinutes: { type: "integer", example: 180 }, priceAmount: { type: "integer", example: 150000 } } } },
    },
  },
  Booking: {
    type: "object",
  ...
  BookingCreateResult: {
    ...
    properties: { booking: ref("Booking"), payment: ref("Payment"), paymentUrl: { type: "string", format: "uri", example: "https://app.pakasir.com/pay/pulih/150000?order_id=PLH-20260201T070000-ABCDEF12&redirect=http%3A%2F%2Flocalhost%3A3001%2Fconsultation%2Fbooking%2Fsuccess" }, instruction: { type: "string", example: "Pay via Pakasir using the link above." } },
  },
  CheckIn: {
    type: "object",
...
const schemaExamples: Record<string, Example> = {
  ...
  CredentialReviewUrl: { fileId: examples.fileId, reviewUrl: null, expiresAt: null, message: "Signed review URL is not configured. Use storage dashboard/manual operations for private review." },
  ...
  AvailabilityWindowResult: { bundles: [{ id: examples.bundleId, profileId: examples.psychologistId, packageName: "Paket 1 Jam", packageDurationMinutes: 60, priceAmount: 150000, dateStart: "2026-06-01", dateEnd: "2026-06-07", dailyStartTime: "09:00:00", dailyEndTime: "12:00:00" }], sessions: [{ id: examples.sessionSlotId, bundleId: examples.bundleId, profileId: examples.psychologistId, sessionDate: "2026-06-03", startsAt: "2026-06-03T09:00:00.000Z", endsAt: "2026-06-03T10:00:00.000Z", status: "available", heldUntil: null, packageName: "Paket 1 Jam", packageDurationMinutes: 60, priceAmount: 150000 }] },
  ...
  PsychologistAvailabilityDate: { date: "2026-06-03", totalSlots: 3, availableSlots: 1, heldSlots: 0, bookedSlots: 2, completedSlots: 0, cancelledSlots: 0, expiredSlots: 0, rescheduledSlots: 0, slots: [{ id: examples.sessionSlotId, startsAt: "2026-06-03T09:00:00.000Z", endsAt: "2026-06-03T12:00:00.000Z", status: "available", packageName: "Paket 3 Jam", packageDurationMinutes: 180, priceAmount: 150000 }] },
  ...
  PaymentStatus: { id: examples.paymentId, bookingId: examples.bookingId, provider: "pakasir", orderId: "PLH-20260201T070000-ABCDEF12", amount: 150000, status: "created", paymentMethod: null, paymentUrl: "https://app.pakasir.com/pay/pulih/150000?order_id=PLH-20260201T070000-ABCDEF12&redirect=http%3A%2F%2Flocalhost%3A3001%2Fconsultation%2Fbooking%2Fsuccess", completedAt: null, expiresAt: "2026-07-16T17:00:00.000Z" },
  Payment: { id: examples.paymentId, bookingId: examples.bookingId, provider: "pakasir", orderId: "PLH-20260201T070000-ABCDEF12", amount: 150000, status: "created", paymentMethod: null, paymentUrl: "https://app.pakasir.com/pay/pulih/150000?order_id=PLH-20260201T070000-ABCDEF12&redirect=http%3A%2F%2Flocalhost%3A3001%2Fconsultation%2Fbooking%2Fsuccess", completedAt: null, expiresAt: "2026-07-16T17:00:00.000Z", createdAt: "2026-07-16T16:00:00.000Z", updatedAt: "2026-07-16T16:00:00.000Z" },
...
const requestSchemas = {
  ...
  PsychologistProfile: { type: "object", additionalProperties: false, required: ["type", "fullName", "dateOfBirth", "address"], properties: { type: { type: "string", enum: ["general", "clinical"], example: "clinical" }, fullName: { type: "string", maxLength: 255, example: "Dr. Demo" }, dateOfBirth: { type: "string", format: "date", example: "1990-01-01" }, address: { type: "string", maxLength: 1000, example: "Jl. Demo No. 1" }, photoUrl: nullableString("https://example.com/photo.jpg", { format: "uri", maxLength: 500000, description: "Optional. Invalid or non-string values are ignored and stored as null." }), bio: nullableString("Licensed clinical psychologist.", { maxLength: 2000 }) } },
  ...
  AvailabilityWindow: { type: "object", additionalProperties: false, required: ["dateStart", "dateEnd", "dailyStartTime", "dailyEndTime", "packages"], properties: { dateStart: date("2026-06-01"), dateEnd: date("2026-06-07"), dailyStartTime: { type: "string", pattern: "^([01]\\d|2[0-3]):[0-5]\\d(:[0-5]\\d)?$", example: "09:00" }, dailyEndTime: { type: "string", pattern: "^([01]\\d|2[0-3]):[0-5]\\d(:[0-5]\\d)?$", example: "12:00" }, packages: { type: "array", minItems: 1, items: { type: "object", additionalProperties: false, required: ["durationMinutes", "priceAmount"], properties: { durationMinutes: { type: "integer", minimum: 1, example: 60 }, priceAmount: { type: "number", minimum: 100000, maximum: 300000, example: 150000 } } } } } },
...
const contracts: Record<string, OperationContract> = {
  ...
  "POST /api/v1/psychologists/me/credential-file": { summary: "Upload psychologist credential file", description: moduleDoc("Uploads private credential file. Allowed: PDF/JPG/JPEG/PNG, max 5 MB. Required docs differ by psychologist type. When all required credential files are uploaded, the profile is automatically approved."), requestBody: body(requestSchemas.CredentialUpload, { clinical_str: { documentType: "str", file: "credential-str.pdf" } }, "multipart/form-data"), successStatus: "201", successMessage: "Credential file uploaded successfully", ...successData("CredentialFile") },
  "POST /api/v1/psychologists/me/submit-for-review": { summary: "Submit psychologist profile for review", description: moduleDoc("Kept for client compatibility. Demo registration already approves profiles; this endpoint validates required profile fields and credential file metadata, then returns an approved profile when requirements are complete."), successMessage: "Psychologist profile submitted for review", ...successData("PsychologistProfile") },
  ...
  "POST /api/v1/psychologists/me/availability-windows": { summary: "Create availability window", description: moduleDoc("Creates one availability window using local dates/times and one or more package definitions. Uses existing session bundle storage for MVP compatibility and generates slots per package duration."), requestBody: body(requestSchemas.AvailabilityWindow, { multi_package: { dateStart: "2026-06-01", dateEnd: "2026-06-07", dailyStartTime: "09:00", dailyEndTime: "12:00", packages: [{ durationMinutes: 60, priceAmount: 150000 }, { durationMinutes: 30, priceAmount: 100000 }] } }), successStatus: "201", successMessage: "Availability window created successfully", successSchema: ref("AvailabilityWindowResult"), successExample: schemaExamples.AvailabilityWindowResult },
  ...
  "GET /api/v1/psychologists/:psychologistId": { summary: "Read approved psychologist detail", description: moduleDoc("Returns public profile for approved psychologist with calendar availability. Draft/rejected profiles are hidden."), successMessage: "Request processed successfully", successSchema: { allOf: [ref("PublicPsychologist"), { type: "object", required: ["availability"], properties: { availability: arr(ref("PsychologistAvailabilityDate")) } }] }, successExample: { ...(schemaExamples.PublicPsychologist as Record<string, unknown>), availability: [schemaExamples.PsychologistAvailabilityDate] } },
  ...
  "GET /api/v1/psychologists/me/availability": { summary: "List my availability dates", description: moduleDoc("Psychologist dashboard endpoint. Returns dates that have generated availability slots with status counts."), successMessage: "Availability dates retrieved successfully", successSchema: arr(ref("PsychologistAvailabilityDate")), successExample: [schemaExamples.PsychologistAvailabilityDate] },
  "GET /api/v1/psychologists/me/bookings/today": { summary: "List today's psychologist bookings", description: moduleDoc("Psychologist dashboard endpoint. Returns bookings assigned to current psychologist where scheduled start date is today in Asia/Jakarta."), successMessage: "Today bookings retrieved successfully", ...successArray("Booking") },
  "POST /api/v1/psychologists/me/bookings/:bookingId/approve": { summary: "Approve booking", description: moduleDoc("Psychologist dashboard endpoint for confirming a paid booking. Clinical sessions require Google Meet link; general sessions ignore meetLink."), requestBody: body(requestSchemas.BookingConfirm, { clinical: { meetLink: "https://meet.google.com/abc-defg-hij" }, general: { meetLink: null } }), successMessage: "Booking approved successfully", ...successData("Booking") },
  "POST /api/v1/psychologists/me/bookings/:bookingId/reschedule": { summary: "Reschedule booking from psychologist dashboard", description: moduleDoc("Psychologist dashboard endpoint. Moves booking to a new available generated slot with reason. MVP applies immediately without patient approval."), requestBody: body(requestSchemas.BookingReschedule, { new_slot: { newSessionSlotId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", reason: "Doctor requested a new time" } }), successMessage: "Booking rescheduled successfully", ...successData("Booking") },
  "POST /api/v1/bookings": { summary: "Create booking", description: moduleDoc("Creates pending-payment booking, claims session slot, blocks overlapping locked slots for the same psychologist, creates Pakasir payment URL, and sets 1-hour expiry."), requestBody: body(requestSchemas.BookingCreate, { slot: { sessionSlotId: examples.sessionSlotId, complaint: "Sulit tidur dan mudah cemas." } }), successStatus: "201", successMessage: "Booking created successfully", successSchema: ref("BookingCreateResult"), successExample: { booking: schemaExamples.Booking, payment: schemaExamples.Payment, paymentUrl: "https://app.pakasir.com/pay/pulih/150000?order_id=PLH-20260201T070000-ABCDEF12&redirect=http%3A%2F%2Flocalhost%3A3001%2Fconsultation%2Fbooking%2Fsuccess", instruction: "Pay via Pakasir using the link above." } },
